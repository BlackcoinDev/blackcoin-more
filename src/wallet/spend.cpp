                           ...
                           bool only_spendable)
{
    ...
    for (const auto& entry : wallet.mapWallet)
    {
        ...
        for (unsigned int i = 0; i < wtx.tx->vout.size(); i++) {
            ...
            COutput coin(outpoint, output, nDepth, input_bytes, spendable, solvable, safeTx, wtx.GetTxTime(), tx_from_me);
...
{
    return AvailableCoins(wallet, coinControl, nMinimumAmount, nMaximumAmount, nMinimumSumAmount, nMaximumCount, /*only_spendable=*/false);
}
        ...
        bool sign) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    coin_selection_params.m_change_fee = GetMinFee(coin_selection_params.change_output_size, GetAdjustedTimeSeconds());
    coin_selection_params.m_cost_of_change = GetMinFee(coin_selection_params.change_spend_size, GetAdjustedTimeSeconds()) + coin_selection_params.m_change_fee;
    ...
    const CAmount not_input_fees = coin_selection_params.tx_noinputs_size ? GetMinFee(coin_selection_params.tx_noinputs_size, GetAdjustedTimeSeconds()) : 0;
    ...
    CAmount fee_needed = GetMinFee(nBytes, GetAdjustedTimeSeconds());
