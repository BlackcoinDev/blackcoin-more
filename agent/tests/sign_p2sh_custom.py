...
def wif_to_privkey(wif):
    ...
    assert payload[0] == 0x99, f"WIF version {payload[0]:02x} != 0x99 (Blackcoin mainnet)"
    if payload[-1] == 0x01:
...
def legacy_sighash(tx, input_idx, redeem_script_hex):
    # Replace the scriptSig of the input being signed with the redeem script,
    # blank all other scriptSigs.
...
def sign_with_privkey(wif, tx, input_idx, redeem_hex):
    ...
    # ecdsa sign_digest appends hash type? No, it returns DER only.
    # Append SIGHASH_ALL
...
def main():
    ...
    with open("/tmp/blackmore_scripttest_manual_signed.json", "w") as f:
