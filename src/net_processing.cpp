 ...
 */
// blackcoin TDDO: do we change to 8 at 50% max outbound of 16? 
static constexpr int32_t MAX_OUTBOUND_PEERS_TO_PROTECT_FROM_DISCONNECT = 4;
/** Timeout for (unprotected) outbound peers to sync to our chainwork */
// blackcoin: change to reflect 2 blocks at 64s block time (Bitcoin default is 2 blocks at 10min block time)
static constexpr auto CHAIN_SYNC_TIMEOUT{128s};
/** How frequently to check for stale tips */
// blackcoin: change to reflect 1 block at 64s block time (Bitcoin default is 1 block at 10min block time)
static constexpr auto STALE_CHECK_INTERVAL{64s};
...
bool PeerManagerImpl::TipMayBeStale()
{
    ...
    return m_last_tip_update.load() < GetTime<std::chrono::seconds>() - std::chrono::seconds{consensusParams.nTargetSpacing} && mapBlocksInFlight.empty();
}
