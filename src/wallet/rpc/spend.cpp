...
RPCHelpMan sendall()
{
    return RPCHelpMan{"sendall",
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            CFeeRate fee_rate{GetMinimumFeeRate(*pwallet, coin_control)};
            ...
            if (options.exists("inputs") && options.exists("send_max")) {
                ...
                for (const COutput& output : AvailableCoins(*pwallet, &coin_control, /*nMinimumAmount=*/0).All()) {
            ...
            const CAmount fee_from_size{std::max(GetMinFee((size_t)tx_size.vsize, GetAdjustedTimeSeconds()), fee_rate.GetFee(tx_size.vsize))};
