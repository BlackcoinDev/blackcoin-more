...
Blackcoin v1/v2
  ...
  ├─ nTime in tx (v1 serialization)
  ├─ nTime in Coin UTXO
  ├─ 16s timestamp mask (nStakeTimestampMask=0xf)
  ├─ nSearchInterval=1 backward search (no-op)
  └─ timer guard (m_last_coin_stake_search_time)
  │
Blackcoin v3/v3.1 (current)
  │
  ├─ v2 txs: nTime stripped from serialization (but still in Coin UTXO)
  ├─ blockFromTime fallback: coin.nTime ?: blockFrom->nTime
  ├─ safety bump for MTP
  └─ optional stake cache with double-check
  │
Qtum (forked from Blackcoin v3)
  │
  ├─ nTime removed from Coin UTXO entirely
  ├─ blockFromTime = blockFrom->nTime (always)
  ├─ forward scan: tries 3 future windows per batch
  ├─ CheckKernelCache: no double-check, no fallback
  ├─ prevoutStake in block header
  ├─ setStakeSeen anti-dupe
  ├─ nReduceBlocktimeHeight fork (adjustable mask, maturity, spacing)
  ├─ delegated staking via EVM (proof-of-delegation)
  └─ multi-cache architecture (miner / wallet / delegate / address)
```
...
### Blackcoin More (`./src/`)
- `pos.cpp` — kernel hash, CheckKernel, CheckProofOfStake, CacheKernel
- `pos.h` — CStakeCache struct, function declarations
- `coins.h` — Coin class with nTime field
- `primitives/transaction.h` — v1/v2 nTime serialization
- `primitives/block.h` — CBlockHeader (no prevoutStake)
- `node/miner.cpp` — CreateNewBlock, PoSMiner loop, safety bump, timer guard
- `wallet/staking.cpp` — CreateCoinStake, backward search, signing
- `wallet/wallet.h` — stakeCache member, staking state
- `validation.cpp` — CheckProofOfStake invocations
- `consensus/params.h` — nStakeTimestampMask, nCoinbaseMaturity, protocol times
- `kernel/chainparams.cpp` — network-specific values

### Qtum (`../qtum/src/`)
- `pos.cpp` — kernel hash, CheckKernel, CheckKernelCache, CheckProofOfStake, CacheKernel
- `pos.h` — function declarations, inline CheckCoinStakeTimestamp
- `qtum/posutils.h` — CStakeCache struct, Delegation struct
- `coins.h` — Coin class (no nTime field)
- `primitives/transaction.h` — same v1/v2 scheme as Blackcoin
- `primitives/block.h` — CBlockHeader with prevoutStake, vchBlockSigDlgt
- `node/miner.cpp` — StakeMiner forward scan, SignBlock, SloveBlock, UpdateMinerStakeCache
- `wallet/stake.cpp` — CreateCoinStakeFromMine, CreateCoinStakeFromDelegate
- `wallet/wallet.h` — minerStakeCache, stakeCache, stakeDelegateCache, fHasMinerStakeCache
- `validation.cpp` — setStakeSeen, CheckProofOfStake invocation
- `consensus/params.h` — StakeTimestampMask, CoinbaseMaturity, nReduceBlocktimeHeight, nOfflineStakeHeight
