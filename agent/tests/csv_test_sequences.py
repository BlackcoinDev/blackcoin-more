...
for seq_label, seq in [("0", 0), ("1", 1), ("0x40000001", 0x40000001), ("0x40000000", 0x40000000), ("0x00010000", 0x00010000)]:
    raw = rpc_wallet("createrawtransaction", [[{"txid":"<CSV_FUNDING_TXID>","vout":0,"sequence":seq}], [{"<RETURN_ADDRESS>":0.999}], 0])
    tx = parse_tx(raw)
    sig = sign(csv_wif, tx, 0, csv_redeem)
    signed = build(tx, 0, sig, csv_redeem)
    test = rpc("testmempoolaccept", [[signed]])
    print(f"nSequence={seq_label} ({seq}): allowed={test[0]['allowed']} reason={test[0].get('reject-reason','')}")
    if test[0]['allowed']:
        print(f"  SIGNED TX: {signed}")
        with open("/tmp/csv_final_tx.txt", "w") as f:
            f.write(signed)
        break
